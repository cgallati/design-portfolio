...
export const SideBySide: FC<TypeSideBySide> = ({ fields }) => {
  ...
  const textComponent = (
    <TextFrame darkMode={darkMode} key={"sbs-text"}>
      <TextWrapper>
        <Title>{title}</Title>
        {body}
      </TextWrapper>
  ...
  const components = photoOnLeft
    ? [assetComponent, textComponent]
    : [textComponent, assetComponent];
  return <SlideFrame>{components}</SlideFrame>;
};
